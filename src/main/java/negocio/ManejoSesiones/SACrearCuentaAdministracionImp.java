	...
	public ResultContext crearCuenta(TCrearCuentaAdm data) {
		...
		if (validarDNI(data.getDni()) || validarNIE(data.getDni())) {
			...
			if (validarPASSWD(data.getConstrasenya())) {
				...
				//el telefono llega en string pero la bd lo acepta como int
				nuevoperfil.append("Telefono", Integer.valueOf(data.getTelf()));
				...
				return new ResultContext(Evento.CREAR_CUENTA_ADM_ERROR_PASSWD_INCORRECTA, null);
			}
