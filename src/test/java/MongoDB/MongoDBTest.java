...
//        assertNotNull("MongoDBManager instance should not be null", mongo);
    ...
    public void testCRUD() {
    	...
    	Document d3 = new Document("nombre", "Juan").append("peso", 28)
    			.append("altura", 145).append("edad", 37);
    	mongo.insertDocument(Collections.TEST_BD, d1);
    	mongo.insertDocument(Collections.TEST_BD, d2);
    	mongo.insertDocument(Collections.TEST_BD, d3);
    	
    	 // Case 1: Read documents from the collection based on the query
        ArrayList<Document> result = mongo.readDocument(d1, Collections.TEST_BD);
        ...
        ArrayList<Document> result2 = mongo.readDocument(queryDocument2, Collections.TEST_BD);
        // Count the number of retrieved documents
